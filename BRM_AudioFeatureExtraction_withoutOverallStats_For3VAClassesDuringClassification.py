...
############################################## Classification in valence and arousal only ########################################################
    ...
    if classType == '2Classes':
        ...
        if valenceFlag == 1:
            for idx_, tr_ in zip(DF.index.values, DF.loc[:, 'target'].values):
                DF.loc[idx_, 'target'] = int(str(tr_)[0]) - 1

        if valenceFlag == 0:
            for idx_, tr_ in zip(DF.index.values, DF.loc[:, 'target'].values):
                DF.loc[idx_, 'target'] = int(str(tr_)[1]) - 1

        classList = np.unique(DF['target'].values.reshape(-1,1))
        ...
        if no_classes == 3:
            ...
            for repeat in np.arange(10):
                ...
                print(f' Train Target 0 = {sum(allTargets.reshape(-1)==0)}')
                print(f' Train Target 1 = {sum(allTargets.reshape(-1)==1)}')
                print(f' Train Target 2 = {sum(allTargets.reshape(-1)==2)}')
...
#########################################################################################################
    ...
    if not os.path.isfile(os.path.join(targetDir, videoPrefix+'AudioFeatureBasedValenceArousalCalculation.csv')):
        ...
        for clip_ in clipsDir:
            ...
            if 'firstArr' in locals():
                del firstArr
            if 'max_' in locals():                
                del max_
            if 'gotIt' in locals():
                del gotIt
            if 'countSim' in locals():
                del countSim
            if 'matchedVid' in locals():
                del matchedVid
            if 'secondArr' in locals():
                del secondArr            

            firstArr = newName2.split('_')
            max_ = 0
            gotIt = 0
            for _vid_ in videoNames:            
                countSim = 0
                secondArr = _vid_.split('_')
                NewsecondArr = []
                if len(secondArr) > 2:
                    for ele_ in secondArr:
                        if not ele_.isnumeric():
                            NewsecondArr.extend([ele_])
                    secondArr = NewsecondArr.copy()
                    del NewsecondArr
                #print(secondArr)

                for first_ in firstArr:                    
                    if first_ in secondArr:
                        countSim = countSim + 1
                ...
                if len(firstArr) > len(secondArr):
                    toCompArr = firstArr
                elif len(firstArr) < len(secondArr):
                    toCompArr = secondArr
                else:
                    toCompArr = secondArr

                if countSim > 0.5*len(toCompArr):

                    print(f'2015 firstArr {firstArr}')
                    print(f'2015 matchedVid {matchedVid}')
                    print(f'2015 newName2 {newName2}')
                    print(f'2015 max {max_}')
                    gotIt = 1
                    if 'Breakdown' == firstArr:
                        pdb.set_trace()
                    break

            #if max_ > 0.75*len(firstArr):
            if gotIt == 1:
...
########################## Calculating Visual Features #####################
                try:
                    ...
                    pdb.set_trace()
                videoFeatureDf.loc[matchedVid, ['max_motion_comp', 'min_motion_comp', 'mean_motion_comp', 'std_motion_comp', 'motion_percent_1', 'motion_percent_2', 'motion_percent_3', 'max_shotRate_', 'min_shotRate_', 'mean_shotRate_', 'std_shotRate_', '_shotRate_percent_1', '_shotRate_percent_2', '_shotRate_percent_3', 'max_rhythm_comp', 'min_rhythm_comp', 'mean_rhythm_comp', 'std_rhythm_comp', 'rhythm_comp_percent_1', 'rhythm_comp_percent_2', 'rhythm_comp_percent_3', 'max_bright_array', 'min_bright_array', 'mean_bright_array', 'std_bright_array', 'bright_array_percent_1', 'bright_array_percent_2', 'bright_array_percent_3', 'max_shotRate', 'min_shotRate', 'mean_shotRate', 'std_shotRate', 'shotRate_percent_1', 'shotRate_percent_2', 'shotRate_percent_3']] = [max_motion_comp, min_motion_comp, mean_motion_comp, std_motion_comp, motion_percent_1, motion_percent_2, motion_percent_3, max_shotRate_, min_shotRate_, mean_shotRate_, std_shotRate_, _shotRate_percent_1, _shotRate_percent_2, _shotRate_percent_3, max_rhythm_comp, min_rhythm_comp, mean_rhythm_comp, std_rhythm_comp, rhythm_comp_percent_1, rhythm_comp_percent_2, rhythm_comp_percent_3, max_bright_array, min_bright_array, mean_bright_array, std_bright_array, bright_array_percent_1, bright_array_percent_2, bright_array_percent_3, max_shotRate, min_shotRate, mean_shotRate, std_shotRate, shotRate_percent_1, shotRate_percent_2, shotRate_percent_3]
...
######################### This data-frame is being constructed for ANN Based prediction of rated valence and arousal from the calculated one.
                    ...
                    audioVideoFeatPCA.loc[matchedVid, valCols] = np.mean(VrescaledPC,0).tolist()  # Taking mean across all the window segments in which audio signal is automatically divided.
                    audioVideoFeatPCA.loc[matchedVid, arlCols] = np.mean(ArescaledPC,0)
                    ...
                    if emotionWise == 0:
                        try:
                            #audioVideoFeatPCA.loc[newName2, 'valence'] = participantRating.loc[matchedVid, ['Valence']].values
                            audioVideoFeatPCA.loc[matchedVid, 'valence'] = participantRating.loc[matchedVid, ['Valence']].values
                        except:
                            pdb.set_trace()

                        #audioVideoFeatPCA.loc[newName2, 'arousal'] = participantRating.loc[newName2, ['Arousal']].values
                        audioVideoFeatPCA.loc[matchedVid, 'arousal'] = participantRating.loc[matchedVid, ['Arousal']].values
                    else:
                        ...
                        audioVideoFeatPCA.loc[matchedVid, 'valence'] = participantRatingGB.loc[matchedVid, ['Valence']].values[0]
                        audioVideoFeatPCA.loc[matchedVid, 'arousal'] = participantRatingGB.loc[matchedVid, ['Arousal']].values[0]  
#################### Encoding the valence-arousal quadrants in 0, 1, 2, and 3 ##################
                    targetName = []
                    if emotionWise == 0:                        
                        flagTar = 0                      
                        for val_, arl_ in zip(audioVideoFeatPCA.loc[matchedVid, 'valence'], audioVideoFeatPCA.loc[matchedVid, 'arousal']):
                            flagTar = 1
                            if (val_ <= 3.5) and (arl_ <= 3.5):
                                targetName.extend(['1'+'1'])
                            elif (val_ <= 3.5) and (arl_ > 3.5 and arl_ <= 6):
                                targetName.extend(['1'+'2'])
                            elif (val_ <= 3.5) and (arl_ > 6):
                                targetName.extend(['1'+'3'])

                            elif (val_ > 3.5 and val_ <= 6) and (arl_ <= 3.5):
                                targetName.extend(['2'+'1'])
                            elif (val_ > 3.5 and val_ <= 6) and (arl_ > 3.5 and arl_ <= 6):
                                targetName.extend(['2'+'2'])
                            elif (val_ > 3.5 and val_ <= 6) and (arl_ > 6):
                                targetName.extend(['2'+'3'])

                            elif (val_ > 6) and (arl_ <= 3.5):
                                targetName.extend(['3'+'1'])
                            elif (val_ > 6) and (arl_ > 3.5 and arl_ <= 6):
                                targetName.extend(['3'+'2'])
                            elif (val_ > 6) and (arl_ > 6):
                                targetName.extend(['3'+'3'])
                            else:
                                pdb.set_trace()
                        ...
                        audioVideoFeatPCA.loc[matchedVid, 'target'] = targetName

                    else:
                        val_ = audioVideoFeatPCA.loc[matchedVid, 'valence']
                        arl_ = audioVideoFeatPCA.loc[matchedVid, 'arousal']
                        if (val_ > 5.0) and (arl_ > 5.0):
                            audioVideoFeatPCA.loc[matchedVid, 'target'] = targetDict['HVHA']
                        if (val_ <= 5.0) and (arl_ > 5.0):
                            audioVideoFeatPCA.loc[matchedVid, 'target'] = targetDict['LVHA']
                        if (val_ <= 5.0) and (arl_ <= 5.0):
                            audioVideoFeatPCA.loc[matchedVid, 'target'] = targetDict['LVLA']
                        if (val_ > 5.0) and (arl_ <= 5.0):
                            audioVideoFeatPCA.loc[matchedVid, 'target'] = targetDict['HVLA']

#################################################################################################################

                    '''valArl.loc[newName2, 'CalcVal'] = valence 
                    valArl.loc[newName2, 'CalcArl'] = arousal '''
                    valArl.loc[matchedVid, 'CalcVal'] = valence 
                    valArl.loc[matchedVid, 'CalcArl'] = arousal                    
                    try:
                        ...
                        valArl.loc[matchedVid, 'RatdVal'] = participantRatingGB.loc[matchedVid, ['Valence']].values[0]
                        valArl.loc[matchedVid, 'RatdArl'] = participantRatingGB.loc[matchedVid, ['Arousal']].values[0]
                        valArl.loc[matchedVid, 'RValStd'] = participantRatingGB.loc[matchedVid, ['ValenceStd']].values[0]
                        valArl.loc[matchedVid, 'RArlStd'] = participantRatingGB.loc[matchedVid, ['ArousalStd']].values[0] * 15 ## Scaling so the size of the dots in scatter plot is increased                                                

                    except:
                        #if len(participantRatingGB.loc[newName2, ['Valence']].values) > 1:
                        if len(participantRatingGB.loc[matchedVid, ['Valence']].values) > 1:
                            ...
                            valArl.loc[matchedVid, 'RatdVal'] = participantRatingGB.loc[matchedVid, ['Valence']].values[0][0]
                            valArl.loc[matchedVid, 'RatdArl'] = participantRatingGB.loc[matchedVid, ['Arousal']].values[0][0]
                            valArl.loc[matchedVid, 'RValStd'] = participantRatingGB.loc[matchedVid, ['ValenceStd']].values[0][0]
                            valArl.loc[matchedVid, 'RArlStd'] = participantRatingGB.loc[matchedVid, ['ArousalStd']].values[0][0] * 15 ## Scaling so the size of the dots in scatter plot is increased                                                    
                        else:
                            pdb.set_trace()
                    ...
                    notIncludedlessLength.extend([newName2])
                    #pdb.set_trace()  
            else:                     
                print(newName2)
                if newName2 not in Count_participantRatingGB[Count_participantRatingGB<ratingThrs].index.values:
                    notIncluded.extend([newName2])
        
            print('##################################### Out')

        if emotionWise == 0:
            if isinstance(audioVideoFeatPCA['target'].iloc[np.where(audioVideoFeatPCA['target']!=0)[0][0]], str):
                (audioVideoFeatPCA['target']==0)
            ...
            allFeatures.drop(allFeatures.index.values[np.where(allFeatures['target']==0)[0]],axis=0, inplace=True)
            ...
            allFeatures.to_csv(os.path.join(targetDir, videoPrefix+'ForClassificaion_AllMultimediaCalculatedPCADimensionsValenceArousal_New.csv'))  ## This file will be used for ANN classification
        else:
            pdb.set_trace()
...
###################### After combining the valence and arousal features #######################
        ...
        allFeatures = pd.read_csv(os.path.join(targetDir, videoPrefix+'ForClassificaion_AllMultimediaCalculatedPCADimensionsValenceArousal_New.csv'), index_col = 0) 
        '''for tar in allFeatures['target']:
            tarStr = str(tar)
            if len(tarStr) < 2:
                allFeatures['target'] =
            else:
                allFeatures['target'] = tarStr'''
